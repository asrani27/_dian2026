        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ date('d/m/Y', strtotime($item->peminjaman->tanggal_pinjam)) }}</td>
                    <td>{{ date('d/m/Y', strtotime($item->tanggal_dikembalikan)) }}</td>
                    <td>{{ $item->peminjaman->mahasiswa->nama ?? '-' }}</td>
                    <td>{{ $item->peminjaman->dosen->nama ?? '-' }}</td>
                    <td>
                        @foreach ($item->peminjaman->peminjamanDetails as $detail)
                        <div class="alat-item">
                            • {{ $detail->alat->nama }} ({{ $detail->jumlah }} unit)
                            <br>
                            <small>Kondisi: {{ $detail->kondisi_kembali ?? '-' }}</small>
                        </div>
                        @endforeach
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="9" class="text-center">Tidak ada data pengembalian pada periode ini</td>
                </tr>
                @endforelse
            </tbody>
        </table>
