...
@section('page-description', 'Manajemen data peminjaman alat laboratorium')
...
<div class="mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-900">Daftar Peminjaman Alat</h2>
                <p class="text-gray-600 mt-1">Kelola data peminjaman alat laboratorium</p>
            </div>
    ...
    <!-- Search dan Filter -->
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-6">
        <form action="{{ route('admin.peminjaman.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-2">Cari Peminjaman</label>
                <div class="relative">
                    <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari berdasarkan kode, nama mahasiswa/dosen, atau status..."
            ...
            <div>
                ...
                <div class="flex space-x-2">
                    ...
                    @if($search ?? null)
                    <a href="{{ route('admin.peminjaman.index') }}" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">
                        Reset
                    </a>
                    @endif
                </div>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm">
                                ...
                                <div class="text-gray-500">{{ $item->dosen->nama ?? '-' }}</div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <span class="text-sm font-medium text-gray-900">{{ $item->peminjamanDetails->count() }}</span>
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <h3 class="text-lg font-medium text-gray-900 mb-2">Belum ada data peminjaman</h3>
                                <p class="text-gray-500 mb-4">Mulai dengan menambahkan data peminjaman alat pertama</p>
                                <a href="{{ route('admin.peminjaman.create') }}"
            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm text-gray-700">
                        Menampilkan <span class="font-medium">{{ $peminjaman->firstItem() }}</span> hingga <span
                            class="font-medium">{{ $peminjaman->lastItem() }}</span> dari <span class="font-medium">{{
                            $peminjaman->total() }}</span> hasil
                    </p>
                </div>
                <div>
                    {{ $peminjaman->links() }}
